      ...
      <template #list="slotProps">
        <div class="flex flex-col">
          <div v-for="(item, index) in slotProps.items" :key="index">
            <div class="flex flex-col sm:flex-row sm:items-center py-2 px-6 gap-4"
              :class="{
              ...
              <div class="flex flex-col md:flex-row justify-between md:items-center flex-1 gap-3">
                <div class="flex flex-row md:flex-col justify-between items-start gap-2">
                  <div>
                    <div class="text-xl font-medium mt-2"><i v-tooltip="item.status" class="pi font-extrabold mr-2" :class="(item.status === 'HECHO') ? 
                    ['pi-check-circle', 'text-green-800'] : 
                    ['pi-minus-circle', 'text-surface-500'] "></i>{{ item.title }}</div>
                    <span
                      class="font-medium text-surface-500 dark:text-surface-400 text-sm"
                      >{{ item.description }}
                    </span>
                    <div class="m-0">
                      <i class="pi pi-calendar cursor-pointer" v-tooltip="'Creado el'"></i>
                      <span class="text-sm text-surface-500 pl-1"> Creado: {{ item.created_at.split('T')[0] }}</span>
                    </div>
                    <div class="m-0">
                      <i class="pi pi-calendar cursor-pointer" v-tooltip="'Fecha vencimiento'"></i>
                      <span class="text-sm text-surface-500 pl-1"> Expira: {{ item.expires ?? 'S/F' }}</span>
                    </div>
                  </div>
                </div>
...
function changeStatus(event, data) {
  const message = data.status === 'PENDIENTE' ? 
...
const tasks = computed(() => { 
  return store.getters.dataTaskTable(sortKey.value)
});
